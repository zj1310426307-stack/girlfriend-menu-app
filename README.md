...
- “和女朋友实时对战”双人大话骰房间
- “今晚转盘”：支持 2～12 个自定义分区，选项保存在当前手机
- 小程序原生“小厨房管理台”：实时查看订单并修改状态

管理端既可以使用小程序首页的“小厨房管理”入口，也可以继续使用网页：
...
`1.0.18` 增加房间计分板：每次开盅后胜者加 1 分，双方再来一局时比分继续保留。叫骰区增加“顺手加码”、骰面图标、合法叫法校验和更清楚的“开！不信”按钮，减少来回点击。

房间状态由 FastAPI WebSocket 服务维护，房间最多两人，并通过邀请码校验。第一版房间保存在单个后端进程的内存中；后端重启或重新部署后，需要重新创建房间。订单、菜品和评价仍保存在 PostgreSQL，不受影响。

### 今晚转盘

首页点击“今晚转盘”进入 `pages/wheel/index`。转盘使用微信原生 Canvas 2D 绘制，不依赖外部图片：

- 支持 2～12 个分区，可随时增加、删除和编辑内容。
- 所有分区等概率，转动过程使用缓出动画并带轻触反馈。
- 自定义内容保存在微信本地缓存 `gf_wheel_items`，重新打开仍然保留；清理小程序缓存后会恢复默认选项。

实现参考了 Apache-2.0 许可的 [buuing/lucky-canvas](https://github.com/buuing/lucky-canvas) 的可配置奖项、高清 Canvas 和“启动后落点”设计思路，但转盘绘制与交互代码为本项目独立实现，没有复制第三方源码或引入额外组件包。

### 小程序管理台

首页点击“小厨房管理”，输入后端环境变量 `ADMIN_PASSWORD` 对应的管理密码即可进入：

- 查看所有已提交订单、菜品数量、备注、希望用餐时间和评价。
- 修改订单状态为待接单、已接单、制作中、已完成或暂时做不了。
- 通过 `/ws/admin/orders` 接收新订单和状态变化；WebSocket 断开时每 12 秒自动刷新作为备用。
- 管理 token 只保存在当前微信本地缓存。退出管理后会立即清除。

个人主体小程序无需配置业务域名；管理台使用已有 `request` 和 `socket` 合法域名直接调用 FastAPI。
...
4. 点击微信开发者工具右上角“上传”，版本号填写当前待发布版本（本地为 `1.0.18`）。
5. 到微信公众平台提交体验版或正式审核。
