...
截至 2026-09-02，已尝试创建免费体验环境 `loveos-staging`（环境 ID：`loveos-staging-d4gchuaw70bdc5234`）。购买页确认配置费用为 0 元、试用期 6 个月、3000 资源点，并明确免费体验版不支持加购资源包或开启按量付费。环境持续显示 `UNAVAILABLE`；费用中心对应订单显示“发货失败已退款”，1 个资源发货失败，折后总价与实付金额均为 0 元。控制台随后调用 `tcb/DescribeUserEnvRes` 返回 `InvalidParameter`，原因为 `env status invalid`，进一步确认环境记录存在但资源未成功交付。因此本次创建未成功完成，需由腾讯云支持确认失败原因和免费体验资格恢复方式；不得删除重建、开启付费或把本路径记为通过。账号标识和完整请求 ID 不写入仓库，只在私下支持渠道按需提供。
