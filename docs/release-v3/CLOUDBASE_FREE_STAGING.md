...
截至 2026-09-02，已创建免费体验环境 `loveos-staging`（环境 ID：`loveos-staging-d4gchuaw70bdc5234`）。购买页确认配置费用为 0 元、试用期 6 个月、3000 资源点，并明确免费体验版不支持加购资源包或开启按量付费。环境当前仍显示 `UNAVAILABLE`，按官方错误说明先等待资源初始化完成；在状态可用、云托管部署和健康检查产生真实证据前，不得把本路径记为通过。
