"""Idempotent completion pipeline for persisted turn-based games."""
...
from notification_service import create_notification_once
...
def _love_tasks(db: Session, record: models.GameRecord, player_ids: list[str]) -> None:
    """Create one stable couple action per human after landlord finishes."""
...
def finalize_game_record(
    ...
    difficulty: str = "rule",
    *,
    include_mode_bonus: bool = True,
) -> models.GameRecord:
    """Finish every side effect through an idempotent retryable state machine.

    The durable record is marked ``pending`` before rewards begin. Every child
    write is keyed by the record ID, so a scheduled retry can safely continue
    after a process or database failure without awarding points twice.
    """
    if record.settlement_status == "complete":
        return record
    ...
    record.result = {**(record.result or {}), "_settlement": "pending"}
    db.commit()
    ...
    try:
        ...
        if include_mode_bonus and winner_id in human_ids:
            participant_ids = [
            ...
            bonus = (
                2
                if against_ai and difficulty in {"random", "rule"}
                else 8
                if against_ai
                else 10
            )
        ...
        db.rollback()
        failed = db.get(models.GameRecord, record.id)
        if failed:
            failed.settlement_status = "failed"
            failed.settlement_error = str(error)[:1000]
            failed.result = {**(failed.result or {}), "_settlement": "failed"}
            db.commit()
        raise
...
def settle_session_game(
    ...
    """Create the round record once, then run the retryable settlement pipeline."""
    existing = (
    ...
    try:
        duration = max(
            0,
            int((datetime.now() - datetime.fromisoformat(started_at)).total_seconds()),
        )
    ...
    return finalize_game_record(db, record, state, winner_id, difficulty)
...
def complete_love_task(
    db: Session,
    customer_id: str,
    task_id: int,
) -> models.LoveTask:
    """Complete one owned post-game promise and award an idempotent bonus."""
    task = (
        db.query(models.LoveTask)
        .filter(
            models.LoveTask.id == task_id,
            models.LoveTask.player_id == customer_id,
        )
        .first()
    )
    if not task:
